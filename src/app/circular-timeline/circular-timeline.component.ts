...
import { Color, colors } from './circular-timeline.model';

@Component({
  selector: 'app-circular-timeline',
  templateUrl: './circular-timeline.component.html',
  styleUrls: ['./circular-timeline.component.scss']
})
export class CircularTimelineComponent implements OnInit, OnDestroy {
  timeout: any;
  counter: number = 0;
  colors: Color[] = colors;

  @Input() timelineData: any;
  @Output() openProof = new EventEmitter<any>();
  constructor() {}
  ...
  createNode = (
    ...
    this.timeout = setTimeout(() => {
      ...
      svg.append(
        this.setProgressIcon(counter, info, cx, cy),
        this.setDateTime(x, y, info)
      );

      const descriptionObject = this.getSVGNode('foreignObject', {
        x: x + 2,
        y: y + 5,
        width: 22,
        height: 12,
      ...
      const descriptionDIV = this.getHTMLNode('div', {
  ...
  setSVGNode = (
    ...
    svg.appendChild(
      this.getSVGNode('rect', {
        x: x,
        y: y,
        width: 25,
        height: 2,
        rx: 1,
        fill: `#${this.colors[counter % colorLength].lighten}`,
        'fill-opacity': '0.62',
        id: `rect${counter}`,
      })
    ); // Bar
    svg.appendChild(
      this.getSVGNode('circle', {
        cx: cx,
        cy: y + 1,
        r: 1.5,
        fill: `#${this.colors[counter % colorLength].darken}`,
      })
    ); // Outer circle
    svg.appendChild(
      this.getSVGNode('circle', { cx: cx, cy: y + 1, r: 0.5, fill: '#FFFFFF' })
    ); // Inner Circle
    svg.appendChild(
      this.getSVGNode('path', {
        d: d,
        stroke: `#${this.colors[counter % colorLength].darken}`,
        'stroke-width': '.2',
      })
    ); // Line
    svg.appendChild(
  ...
  setHalfCircle = (x: number, y1: number, y2: number) => {
    this.timeout = setTimeout(() => {
      ...
      const arc = this.getSVGNode('path', {
        d: d,
        'stroke-width': 2,
        stroke: '#C4C4C4',
        fill: 'none',
  ...
  setDateTime = (
    x: number,
    y: number,
    info: { title?: any; date?: any; time?: any }
  ) => {
    const date = this.getSVGNode('text', {
      x: x + 3,
      y: y + 5,
      fill: 'brown',
      id: 'svg-date',
  ...
  setProgressIcon = (
    ...
    const icon = this.getSVGNode('text', {
      x: cx - 1.5,
      y: cy + 3,
      width: 1,
      height: 1,
  ...
  setEndCircle = (color: string, currentText: string) => {
    ...
    const text = this.getSVGNode('text', {
      x: 10,
      y: 18.8,
      width: 10,
      height: 2,
  ...
  setStartCircle = (dir: number) => {
    this.timeout = setTimeout(() => {
      ...
      const lastRect = <SVGGraphicsElement>(
        document.querySelector(`#rect${number}`)
      );
      const svg = document.getElementById('svg-container') as HTMLElement;
      const position = lastRect.getBBox();
      svg.appendChild(
        this.getSVGNode('circle', {
          cx: position.x - 1 * xPosition,
          cy: position.y + 1,
          r: '5',
          fill: '#ffe6ca',
          stroke: '#FF7E21',
